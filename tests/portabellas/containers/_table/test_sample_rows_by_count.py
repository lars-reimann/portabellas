...
class TestHappyPath:
    def test_should_sample_rows_by_count(
        ...
        actual = table_factory().sample_rows_by_count(count)
    ...
    def test_should_not_mutate_receiver(
        ...
        original.sample_rows_by_count(count)
...
class TestWithReplacement:
    ...
    def test_should_sample_rows_by_count_with_replacement(
        self,
        table_factory: Callable[[], Table],
        count: int,
        expected: Table,
    ) -> None:
        actual = table_factory().sample_rows_by_count(count, with_replacement=True)
        assert_tables_are_equal(actual, expected)

    def test_should_allow_count_exceeding_row_count_with_replacement(self) -> None:
        table = Table({"col1": [1, 2, 3]})
        result = table.sample_rows_by_count(10, with_replacement=True)
...
class TestRandomSeed:
    ...
    def test_should_produce_different_result_with_different_seed(self) -> None:
        ...
        result1 = table.sample_rows_by_count(50, random_seed=1)
        result2 = table.sample_rows_by_count(50, random_seed=2)
...
class TestValidation:
    def test_should_return_empty_table_if_count_is_zero(self) -> None:
        table = Table({"col1": [1, 2, 3]})
        result = table.sample_rows_by_count(0)
