...
class TestShouldCreateCsvFile:
    def test_path_as_string(self, table: Table, tmp_path: Path) -> None:
        ...
        table.write.csv_file(path_as_string)
        restored = Table.read.csv_file(path_as_string)
        assert restored == table

    def test_path_as_path_object(self, table: Table, tmp_path: Path) -> None:
        path_as_path_object = tmp_path / "table.csv"

        table.write.csv_file(path_as_path_object)
        restored = Table.read.csv_file(path_as_path_object)
        assert restored == table
...
def test_should_add_missing_extension(tmp_path: Path) -> None:
    ...
    table = Table({})
    table.write.csv_file(write_path)
    restored = Table.read.csv_file(read_path)
    assert restored == table


def test_should_raise_if_wrong_file_extension(tmp_path: Path) -> None:
    with pytest.raises(FileExtensionError):
        Table({}).write.csv_file(tmp_path / "table.txt")


def test_should_create_parent_directories(tmp_path: Path) -> None:
    path = tmp_path / "subdir" / "table.csv"

    Table({"a": [1]}).write.csv_file(path)
