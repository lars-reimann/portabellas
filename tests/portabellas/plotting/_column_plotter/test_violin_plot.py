...
class TestViolinPlot:
    def test_should_create_violin_plot(self, column: Column, config: PlotConfig | None, expected_title: str) -> None:
        plot = column.plot.violin_plot(config=config)
...
def test_should_set_y_axis_to_log() -> None:
    ...
    plot = column.plot.violin_plot(y_axis=AxisConfig(log=True))
