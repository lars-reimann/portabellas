...
class TestBoxPlots:
    def test_should_create_box_plots(self, table: Table, title: str | None) -> None:
        plot = table.plot.box_plots(config=PlotConfig(title=title))
        ...
        for i in range(expected_count):
            assert plot._figure.data[i].type == "box"
...
def test_should_set_y_axis_to_log() -> None:
    ...
    plot = table.plot.box_plots(y_axis=AxisConfig(log=True))
