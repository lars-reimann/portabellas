...
class StructOperations(ABC):
    ...
    --------
        ...
        >>> column.map(lambda cell: cell.struct.rename("name", "fname").struct.to_json())
        +----------------------------+
        | a                          |
        | ---                        |
        | str                        |
    ...
    def get(self, name: str) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [{"name": "Alice", "age": 25}, {"name": "Bob", "age": 30}])
        >>> column.map(lambda cell: cell.struct.prefix_names("p_").struct.to_json())
        +-------------------------------+
        | a                             |
        | ---                           |
        | str                           |
        +===============================+
        | {"p_name":"Alice","p_age":25} |
        | {"p_name":"Bob","p_age":30}   |
        +-------------------------------+
        """
    ...
    def suffix_names(self, suffix: str) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [{"name": "Alice", "age": 25}, {"name": "Bob", "age": 30}])
        >>> column.map(lambda cell: cell.struct.suffix_names("_s").struct.to_json())
        +-------------------------------+
        | a                             |
        | ---                           |
        | str                           |
        +===============================+
        | {"name_s":"Alice","age_s":25} |
        | {"name_s":"Bob","age_s":30}   |
        +-------------------------------+
        """
