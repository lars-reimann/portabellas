...
class ListOperations(ABC):
    ...
    def get(self, index: ConvertibleToIntCell) -> Cell:
        ...
        Negative indices count from the end of the list (-1 is the last element). If the index is out of bounds,
        the result is null.

        Parameters
        ----------
        index:
            The index of the value to get. Negative indices count from the end.
