...
class MathOperations(ABC):
    ...
    def acos(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.acos())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def acosh(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.acosh())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def asin(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.asin())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def asinh(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.asinh())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def atan(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.atan())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def atanh(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.atanh())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def cbrt(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.cbrt())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def ceil(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.ceil())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def cos(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.cos())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def cosh(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.cosh())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def degrees_to_radians(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.degrees_to_radians())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def exp(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.exp())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def floor(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [1.1, 3.0, None])
        >>> column.map(lambda cell: cell.math.floor())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def log(self, base: float) -> Cell:
        ...
        ------
        OutOfBoundsError
            If the base is less than or equal to 0.
        ValueError
            If the base is equal to 1.
    ...
    def log10(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.log10())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def radians_to_degrees(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [0, math.pi / 2, math.pi, 3 * math.pi / 2, None])
        >>> column.map(lambda cell: cell.math.radians_to_degrees())
        +-----------+
        |         a |
        |       --- |
        |       f64 |
    ...
    def round_to_decimal_places(self, decimal_places: int) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.round_to_decimal_places(0))
        +---------+
        |       a |
        |     --- |
        |     f64 |
        ...
        >>> column.map(lambda cell: cell.math.round_to_decimal_places(2))
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def round_to_significant_figures(self, significant_figures: int) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [0.999, 1.123, 3.456, None])
        >>> column.map(lambda cell: cell.math.round_to_significant_figures(1))
        +---------+
        |       a |
        |     --- |
        |     f64 |
        ...
        >>> column.map(lambda cell: cell.math.round_to_significant_figures(2))
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def sign(self) -> Cell:
        ...
        Note that IEEE 754 defines a negative zero (-0) and a positive zero (+0). This method returns a negative zero
        ...
        >>> column1.map(lambda cell: cell.math.sign())
        +------+
        |    a |
        |  --- |
        |  i64 |
        ...
        >>> column2.map(lambda cell: cell.math.sign())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def sin(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [0, math.pi / 2, math.pi, 3 * math.pi / 2, None])
        >>> column.map(lambda cell: cell.math.sin())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def sinh(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.sinh())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def sqrt(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.sqrt())
        +---------+
        |       a |
        |     --- |
        |     f64 |
    ...
    def tan(self) -> Cell:
        ...
        >>> column.map(lambda cell: cell.math.tan())
        +----------+
        |        a |
        |      --- |
        |      f64 |
    ...
    def tanh(self) -> Cell:
        ...
        >>> from portabellas import Column
        >>> column = Column("a", [-1, 0, 1, None])
        >>> column.map(lambda cell: cell.math.tanh())
        +----------+
        |        a |
        |      --- |
        |      f64 |
